...
export const AppLayout = () => {
    ...
    const selectedPermissionData = selectedPermission ? permissions.find((p) => p.name === selectedPermission) || null : null;

    const selectedPermissionsData = selectedForComparison.map((name) => permissions.find((p) => p.name === name)).filter(Boolean) as typeof permissions;

    const handleComparisonToggle = (permissionName: string) => {
        setSelectedForComparison((prev) => (prev.includes(permissionName) ? prev.filter((name) => name !== permissionName) : [...prev, permissionName]));
    };
        ...
        return (
            <Layout style={{ minHeight: '100vh' }}>
                ...
                <Content style={{ padding: '24px' }}>
                    <Alert message='Error Loading Permissions' description={error} type='error' showIcon />
                </Content>
            </Layout>
        );
    }
    ...
    return (
        <Layout style={{ minHeight: '100vh' }}>
            <Header style={{ padding: 0 }}>
                <TitleBar apiVersion={apiVersion} onApiVersionChange={setApiVersion} hideDisabled={hideDisabled} onHideDisabledChange={setHideDisabled} />
            </Header>
