...
export const PermissionComparison = ({ selectedPermissions, descriptions, onRemovePermission }: PermissionComparisonProps) => {
    const [showDifferencesOnly, setShowDifferencesOnly] = useState(false);
    const getSchemeIcon = (schemeType: string) => {
        switch (schemeType) {
            case 'Application': return <AppstoreOutlined />;
            case 'DelegatedWork': return <UserOutlined />;
            case 'DelegatedPersonal': return <UserOutlined />;
    ...
    const getSchemeColor = (schemeType: string) => {
        switch (schemeType) {
            case 'Application': return '#722ed1';
            case 'DelegatedWork': return '#1890ff';
            case 'DelegatedPersonal': return '#fa8c16';
    ...
    const endpoints = showDifferencesOnly 
        ? allEndpoints.filter(hasAccessDifferences)
        : allEndpoints;
        ...
        ...selectedPermissions.map(permission => ({
            title: (
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <Text strong style={{ fontSize: '11px' }}>{permission.name}</Text>
                    <Button
                        type="text"
                        size="small"
                        icon={<CloseOutlined />}
        ...
        <div style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '16px', marginBottom: '24px' }}>
                {selectedPermissions.map(permission => {
                    ...
                    const maxPrivilegeLevel = Math.max(
                        ...
                        <Card 
                            ...
                            title={
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                    <Text strong style={{ fontSize: '14px' }}>{permission.name}</Text>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                        <PrivilegeLevelIndicator level={maxPrivilegeLevel} />
                                        <Button
                                            type="text"
                                            size="small"
                                            icon={<CloseOutlined />}
